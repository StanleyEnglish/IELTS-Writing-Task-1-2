...
import {
  IELTS_TASK_1_BAND_DESCRIPTORS,
  IELTS_TASK_2_BAND_DESCRIPTORS,
} from "../constants";
...
export const generateGuidance = async (taskType, prompt, imageBase64) => {
  try {
        ...
        responseSchema: {
          ...
          properties: {
            points: {
              type: Type.ARRAY,
              description: isTask1
                ? "An array of 3-4 key features to report on."
                : "An array of two brainstorming questions in Vietnamese.",
    ...
    throw new Error(
      "The AI failed to generate guidance. This might be a temporary issue. Please try again."
    );
  }
};
...
export const generateBrainstormingIdeas = async (prompt, questions) => {
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction:
          "You are an expert IELTS writing instructor. Your task is to provide brainstorming ideas for a student's Writing Task 2 essay.",
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            ideas: {
              type: Type.ARRAY,
              description:
                "An array of bullet-pointed ideas, starting with '-'.",
              items: { type: Type.STRING },
            },
          },
          required: ["ideas"],
    ...
    throw new Error(
      "Failed to generate ideas. This could be a temporary issue, please try again."
    );
  }
};
...
export const getIeltsFeedback = async (
  ...
  try {
    const isTask1 = taskType === "Task 1";
    const bandDescriptors = isTask1
      ? IELTS_TASK_1_BAND_DESCRIPTORS
      : IELTS_TASK_2_BAND_DESCRIPTORS;
    const taskCompletionCriterion = isTask1
      ? "Task Achievement"
      : "Task Response";
    ...
    const baseFeedbackProperties = {
      strengths: {
        type: Type.STRING,
        description: "Positive feedback on the criterion.",
      },
      weaknesses: {
        type: Type.STRING,
        description: "Actionable areas for improvement on the criterion.",
      },
    };
    ...
    const mistakeSchema = {
      ...
      properties: {
        originalPhrase: {
          type: Type.STRING,
          description: "The incorrect phrase from the essay.",
        },
        suggestedCorrection: {
          type: Type.STRING,
          description: "The corrected version of the phrase.",
        },
        explanation: {
          type: Type.STRING,
          description: "A brief explanation of the mistake.",
        },
      },
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            taskCompletionScore: {
              type: Type.INTEGER,
              description: `An integer band score from 5-9 for ${taskCompletionCriterion}.`,
            },
            coherenceCohesion: {
              type: Type.OBJECT,
              properties: {
                ...baseFeedbackProperties,
                referencingAndSubstitution: {
                  type: Type.STRING,
                  description:
                    "Specific feedback on referencing and substitution.",
                },
              },
              required: ["strengths", "weaknesses"],
            },
            coherenceCohesionScore: {
              type: Type.INTEGER,
              description:
                "An integer band score from 5-9 for Coherence & Cohesion.",
            },
            lexicalResource: {
              type: Type.OBJECT,
              properties: {
                ...
                mistakes: {
                  type: Type.ARRAY,
                  description:
                    "List of specific lexical mistakes and corrections. Returns empty array if none.",
                  items: mistakeSchema,
                },
              },
              required: ["strengths", "weaknesses"],
            ...
            lexicalResourceScore: {
              type: Type.INTEGER,
              description:
                "An integer band score from 5-9 for Lexical Resource.",
            },
            grammaticalRange: {
              type: Type.OBJECT,
              properties: {
                ...
                mistakes: {
                  type: Type.ARRAY,
                  description:
                    "List of specific grammatical mistakes and corrections. Returns empty array if none.",
                  items: mistakeSchema,
                },
              },
              required: ["strengths", "weaknesses"],
            ...
            grammaticalRangeScore: {
              type: Type.INTEGER,
              description:
                "An integer band score from 5-9 for Grammatical Range & Accuracy.",
            },
            sentenceImprovements: {
              type: Type.ARRAY,
              description:
                "A list of suggestions for ALL sentences that are awkward or unnatural.",
              items: {
                type: Type.OBJECT,
                properties: {
                  originalSentence: {
                    type: Type.STRING,
                    description: "The original sentence from the user's essay.",
                  },
                  suggestedSentence: {
                    type: Type.STRING,
                    description:
                      "The rewritten, more natural-sounding sentence.",
                  },
                },
    ...
    throw new Error(
      "Failed to get feedback from the AI. This may be a temporary issue. Please try again."
    );
  }
};
